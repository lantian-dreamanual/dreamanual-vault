 ...
 *
 * ⚠️ 这个文件里的一切都必须是**虚构**的，四条硬规矩：
 *    域名用 RFC 2606 的 example.com、IP 用 RFC 5737 的文档保留段
 *    （192.0.2.0/24 · 198.51.100.0/24 · 203.0.113.0/24）、
 *    用户名一律 `demo.*`、密码一律 `Demo#` 前缀。
 *
 * 真实信息（公司域名、内网段、生产服务器地址、账号名、私人路径、私人笔记）
 * **不进这个文件**。仓库是公开的，而这个文件会被打进前端产物、进而进二进制与
 * 安装包 —— 2026-09-24 就是这么发现第一批旧数据已经跟着 v1.0.0 发布出去的。
 * 加数据之前先照着上面四条编。 */
